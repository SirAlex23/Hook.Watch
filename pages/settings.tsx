...
import { ArrowLeft, Mail, Building2, Trash2, Plus, LogOut, AlertCircle, CheckCircle2, Loader2 } from 'lucide-react';
...
export default function Settings() {
  ...
  const saveConfig = () => {
    if (!companyName || !adminEmail) {
      showMsg('error', 'RELLENA TODOS LOS CAMPOS DE ESTRATEGIA');
      return;
    }
    if (!validateEmail(adminEmail)) {
      showMsg('error', 'EMAIL DE AVISOS NO VÁLIDO');
      return;
    }
    localStorage.setItem('hook_config', JSON.stringify({ companyName, adminEmail, emailTpl }));
    showMsg('success', 'ESTRATEGIA GUARDADA CORRECTAMENTE');
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] text-white p-6 md:p-10 font-sans selection:bg-emerald-500/30">
      <div className="max-w-4xl mx-auto">
        
        {/* Sistema de Avisos Flotantes */}
        {msg.text && (
          ...
          <div className="space-y-6">
            <section className="bg-[#16181d] border border-white/5 rounded-[2rem] p-8">
              ...
              <div className="grid gap-4">
                <input type="text" value={companyName} onChange={(e) => setCompanyName(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-sm font-bold outline-none focus:border-emerald-500 transition-all" placeholder="Empresa a suplantar (Ej: Microsoft, Amazon, Entidad Propia...)" />
                <input type="email" value={adminEmail} onChange={(e) => setAdminEmail(e.target.value)} className="w-full bg-white/5 border border-white/10 rounded-xl p-4 text-sm font-bold outline-none focus:border-emerald-500 transition-all" placeholder="Email para recibir avisos de inicio y éxito de ataques" />
              </div>
            </section>
            ...
            <button onClick={saveConfig} className="w-full bg-emerald-500 text-black py-5 rounded-2xl font-black uppercase tracking-widest text-xs hover:bg-emerald-400 transition-all shadow-xl shadow-emerald-500/20 mb-4">Guardar Configuración</button>
          ...
          <div className="bg-[#16181d] border border-white/5 rounded-[2.5rem] p-10">
            <div className="grid md:grid-cols-3 gap-4 mb-10">
              ...
              <button 
                onClick={addTarget} 
                disabled={loading}
                className="bg-white text-black rounded-xl font-black uppercase text-[10px] tracking-widest hover:bg-gray-200 transition-all flex items-center justify-center"
              >
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {targets.length === 0 && (
                <div className="col-span-full py-10 text-center border-2 border-dashed border-white/5 rounded-[2rem]">
                  <p className="text-[10px] font-black text-white/20 uppercase tracking-[0.3em]">No hay objetivos registrados</p>
                </div>
              )}
            </div>
          </div>
